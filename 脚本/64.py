...
pwd = 'admin'
port = '22'
local_dir = "C:/Users/root/Desktop/2017-10-16"
remote_dir = '/usr/local/tomcatnew/webapps/'
...
def print_tomcat_logs():
    try:
        ...
        ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh_client.connect(hostname=hostname, port=int(port), username=user, password=pwd)
        std_in, std_out, std_err = ssh_client.exec_command('tail -f /usr/local/tomcatnew/logs/catalina.out')
        # for line in std_out:
        #     print (line.strip("\n"))
        while True:
            print(std_out.readline())
...
def docker_restart():
    try:
        ssh_client = paramiko.SSHClient()
        ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh_client.connect(hostname='192.168.1.64', port=22, username=user, password=pwd)
